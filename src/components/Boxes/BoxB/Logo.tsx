...
export default function Logo() {
  ...
  function renderLogoAccordingToSize() {
    if (largerThan3800) {
      return <BiCake size={90} />;
    } else if (largerThan2500) {
      return <BiCake size={80} />;
    } else if (largerThan1900) {
      return <BiCake size={70} />;
    } else if (largerThan1200) {
      return <BiCake size={60} />;
    } else if (largerThan900) {
      return <BiCake size={50} />;
    } else {
      return <BiCake size={40} />;
    }
  }
  return (
    <div className={styles.logo}>
      <div className={styles.logoIcon}>{renderLogoAccordingToSize()}</div>
      <div className={styles.logoText}>RHOAM RECIPES</div>
    </div>
  );
}
